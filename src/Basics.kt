 ...
 */
fun inRange(start: Int, end: Int): IntRange = IntRange(start, end)

/**
 * Write a function that returns a Range type of n odd integers starting at 1.
 *
 * @param n the number of odd integers in the range
 * @return a Range of odd integers, excluding the last add integer
 */
fun oddRange(n: Int, limit: Int): IntProgression = 1..limit step 2
...
fun minWhile(r: Array<Int>): Int {
    var i = 0
    var minVal = r[0]
    while (i < r.size) if (r[i] >= minVal) i++ else {
        minVal = r[i]
        i++
    }
    return minVal
...
fun minFor(r: Array<Int>): Int {
    var minVal = r.first()
    for (n in r) when { n < minVal -> minVal = n }
    return minVal
}
...
fun minRecursive(r: Array<Int>): Int {
    return if (r.size == 1) {
        ...
        if (minRecursive(r.sliceArray(0 until r.size-1)) <= r[r.size-1]) {
            minRecursive(r.sliceArray(0 until r.size-1))
        } else {
            r[r.size-1]
        }
    }
 ...
 */
fun splitInHalf(s: String): Pair<String, String> = Pair(s.substring(0, s.length/2), s.substring(s.length/2))
...
fun isPalindrome(s: String): Boolean {
    val clean = s.toLowerCase().replace(Regex("[^\\w]"), "")
    return clean == StringBuilder(clean).reverse().toString()
}
 ...
 */
fun wordCounter(lines: Array<String>): Map<String, Int> = lines.map { x -> x.toLowerCase()
        .replace(Regex("[^\\w\\s]"), "")
        .split(" ") }
        .flatten()
